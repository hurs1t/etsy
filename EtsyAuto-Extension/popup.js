...
async function handleGenerate() {
  ...
  try {
    ...
    console.log(`[EtsyAuto] Attempting to reach API at: ${API_URL}`);

    // Quick ping to check connectivity
    try {
      await fetch(`${API_URL}/auth/status`, { mode: 'no-cors' }).catch(() => { });
    } catch (e) { }
    ...
    if (response.error) {
      if (response.error.includes('fetch')) {
        throw new Error(currentLang === 'tr'
          ? 'Sunucuya bağlanılamadı. Lütfen API URL ayarlarını kontrol edin veya sunucunun çalıştığından emin olun.'
          : 'Failed to reach server. Please check API URL settings or ensure server is running.');
      }
